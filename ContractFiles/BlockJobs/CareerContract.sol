...
contract CareerContract {
   ...
   event transferFrom_event(address from, address to, uint amout);
    ...
    // 송금 헤드헌팅 등에 사용... 따로 불변 데이터 저장이 필요 없어보여서 공용 사용
   ...
   function createCareer (string[] memory _role, string memory _description, address _company, uint _stDt, uint _fnsDt, address _admin, uint _amount) public payable{
     // @Exception
       ...
       BlockJobCoinAddress.transferFrom(msg.sender, _admin, _amount);
   ...
   function approveCareer(uint _careerId, address _admin, uint _amount, uint _status) public payable{
     ...
     // 승인 시에 근로자가 등록할 때 admin에게 지불한 값 80%는 기업에게 배분 (가스비가...?)
       ...
       BlockJobCoinAddress.transferFrom(_admin, msg.sender, _amount * 8/10);
     }
